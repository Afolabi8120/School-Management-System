...
{
    public partial class frmExamScore2 : Form
    {
        MySqlCommand cm;
        MySqlConnection cn;
        MySqlDataReader dr;
        ...
        //gets all the numbers of subject for the selected term
        ...
        public void LoadRecord()
        {
            ...
            {
                i++;
                dataGridView1.Rows.Add(i, dr["subject"].ToString(), dr["ca"].ToString(), dr["test"].ToString(), dr["exam"].ToString(), dr["total"].ToString(), dr["grade"].ToString(), dr["firstterm"].ToString(), dr["secondterm"].ToString(), dr["termtotal"].ToString(), dr["ave"].ToString());
            }
            dr.Close();
            cn.Close();
        }
            ...
            try
            {
                ...
                if (MessageBox.Show("Add Score(s) To Mark Sheet! Click Yes to Confirm", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    else
                    {
                        cn.Close();
                        cn.Open();
                        cm = new MySqlCommand("INSERT INTO tblexamscore (admissionno,name,class,term,section,subject,ca,test,exam,total,grade,status,firstterm,secondterm,termtotal,ave) VALUES(@admissionno,@name,@class,@term,@section,@subject,@ca,@test,@exam,@total,@grade,@status,@firstterm,@secondterm,@termtotal,@ave)", cn);
                        cm.Parameters.AddWithValue("@admissionno", txtAdmissionNo.Text);
                        ...
                        cm.Parameters.AddWithValue("@firstterm", txtFirstTermScore.Text);
                        cm.Parameters.AddWithValue("@secondterm", txtSecondTermScore.Text);
                        cm.Parameters.AddWithValue("@termtotal", txtAllTermTotal.Text);
                        cm.Parameters.AddWithValue("@ave", txtTermAverage.Text);
                        cm.ExecuteNonQuery();
                        cn.Close();
                        LoadRecord();
                        getTotalSubject();
                        ...
                        txtExam.Text = "0";
                        txtTotal.Text = "0";
                        txtFirstTermScore.Text = "0";
                        txtSecondTermScore.Text = "0";
                        txtAllTermTotal.Text = "0";
                        txtTermAverage.Text = "0.00";
                        txtGrade.Text = "";
                    }
                    dr.Close();
                    cn.Close();
        ...
        void getFirstTermScore()
        {
            cn.Open();
            cm = new MySqlCommand("SELECT * FROM tblexamscore WHERE admissionno=@admissionno AND class=@class AND term=@term AND section=@section AND subject=@subject", cn);
            cm.Parameters.AddWithValue("@admissionno", txtAdmissionNo.Text);
            cm.Parameters.AddWithValue("@class", txtClass.Text);
            cm.Parameters.AddWithValue("@term", "First Term");
            cm.Parameters.AddWithValue("@section", cboSection.Text);
            cm.Parameters.AddWithValue("@subject", cboSubject.Text);
            dr = cm.ExecuteReader();
            dr.Read();
            if (dr.HasRows)
            {
                txtFirstTermScore.Text = dr["total"].ToString();
            }
            else
            {
                txtFirstTermScore.Text = "0";
        ...
        void getSecondTermScore()
        {
            cn.Open();
            cm = new MySqlCommand("SELECT * FROM tblexamscore WHERE admissionno=@admissionno AND class=@class AND term=@term AND section=@section AND subject=@subject", cn);
            cm.Parameters.AddWithValue("@admissionno", txtAdmissionNo.Text);
            cm.Parameters.AddWithValue("@class", txtClass.Text);
            cm.Parameters.AddWithValue("@term", "Second Term");
            cm.Parameters.AddWithValue("@section", cboSection.Text);
            cm.Parameters.AddWithValue("@subject", cboSubject.Text);
            dr = cm.ExecuteReader();
            dr.Read();
            if (dr.HasRows)
            {
                txtSecondTermScore.Text = dr["total"].ToString();
            }
            else
            {
                txtSecondTermScore.Text = "0";
        ...
        private void txtExam_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                var gettotal = Convert.ToInt32(txtTotal.Text) + Convert.ToInt32(txtFirstTermScore.Text) + Convert.ToInt32(txtSecondTermScore.Text);
                txtAllTermTotal.Text = gettotal.ToString();
                decimal ave = (Convert.ToDecimal(txtAllTermTotal.Text) / 3);
                txtTermAverage.Text = Convert.ToDecimal(ave).ToString();

                if (Convert.ToDecimal(txtTermAverage.Text) >= 70 && Convert.ToDecimal(txtTermAverage.Text) <= 100)
                {
                    txtGrade.Text = "EXCELLENT";
                }
                else if (Convert.ToDecimal(txtTermAverage.Text) >= 50 && Convert.ToDecimal(txtTermAverage.Text) <= 69)
                {
                    txtGrade.Text = "CREDIT";
                }
                else if (Convert.ToDecimal(txtTermAverage.Text) >= 40 && Convert.ToDecimal(txtTermAverage.Text) <= 49)
                {
                    txtGrade.Text = "PASS";
                }
                else if (Convert.ToDecimal(txtTermAverage.Text) >= 0 && Convert.ToDecimal(txtTermAverage.Text) <= 39)
                {
                    txtGrade.Text = "FAIL";
                }
            }
        ...
        private void btnSavePrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cboClass.Focus();
                    MessageBox.Show("Please select the class the student will be promoted to!", "", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                ...
                if (MessageBox.Show("Save Score(s) To Mark Sheet! Click Yes to Confirm", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    else
                    {
                        ...
                        cn.Close();
                        cn.Open();
                        cm = new MySqlCommand("INSERT INTO tblexamlist (admissionno,name,class,gender,dob,picture,term,section,open,present,absent,termbegin,termend,nexttermbegin,principal,teacher,noinclass,totalscores,totalsubject,percentage,remark,date,promotedto) VALUES(@admissionno,@name,@class,@gender,@dob,@picture,@term,@section,@open,@present,@absent,@termbegin,@termend,@nexttermbegin,@principal,@teacher,@noinclass,@totalscores,@totalsubject,@percentage,@remark,@date,@promotedto)", cn);
                        cm.Parameters.AddWithValue("@admissionno", txtAdmissionNo.Text);
                        cm.Parameters.AddWithValue("@name", txtName.Text);
                        cm.Parameters.AddWithValue("@class", txtClass.Text);
                        ...
                        cm.Parameters.AddWithValue("@promotedto", cboClass.Text);
                        cm.ExecuteNonQuery();
                        cn.Close();
                        ...
                        cn.Open();
                        cm = new MySqlCommand("UPDATE tblstudent SET class=@class WHERE admissionno=@admissionno", cn);
                        cm.Parameters.AddWithValue("@admissionno", txtAdmissionNo.Text);
        ...
        private void txtTermAverage_TextChanged(object sender, EventArgs e)
        {
            try
            {
                int total = Convert.ToInt32(txtCA.Text) + Convert.ToInt32(txtTest.Text) + Convert.ToInt32(txtExam.Text);
